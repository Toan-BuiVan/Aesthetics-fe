...
function CartItem({ item, onQuantityChange, onDelete, onAddToInvoice }) {
    ...
    return (
        <div className={cx('cart-item')}>
            <img
                src={`https://buitoandev.somee.com/Images/${item.productImages}`}
...
function ItemCartproduct({ onAddToInvoice, onCheckoutAll }) {
    ...
    useEffect(() => {
        ...
        const fetchCartItems = async () => {
            ...
            try {
                const response = await fetch('https://buitoandev.somee.com/api/CartProduct/GetList_SearchCartProduct', {
                    method: 'POST',
                    headers,
    ...
    const handleDeleteItem = async (cartProductID) => {
        ...
        try {
            const response = await fetch('https://buitoandev.somee.com/api/CartProduct/Delete_CartProduct', {
                method: 'DELETE',
                headers,
                body: JSON.stringify(requestData),
            });
    ...
    const handleUpdateQuantity = async (cartProductID, newQuantity) => {
        ...
        try {
            const response = await fetch('https://buitoandev.somee.com/api/CartProduct/Update_CartProduct', {
                method: 'POST',
                headers,
                body: JSON.stringify(requestData),
            });
