...
function ProductDetailsPage({ product, onBack, onSelectProduct }) {
    ...
    const handlePayment = async () => {
        ...
        const apiUrl = 'https://buitoandev.somee.com/api/Invoice/Insert_Invoice';
        try {
            const response = await fetch(apiUrl, {
                method: 'POST',
                headers: headers,
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const response = await fetch('https://buitoandev.somee.com/api/Comment/GetList_SearchComment', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    ...
                    <div className={cx('related-product-list')}>
                        {relatedProducts.map((relatedProduct) => (
                            <div key={relatedProduct.productID} className={cx('related-product-item')}>
                                <img
                                    src={`https://buitoandev.somee.com/Images/${relatedProduct.productImages}`}
