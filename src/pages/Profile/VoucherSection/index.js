...
function VoucherSection() {
    ...
    useEffect(() => {
        const fetchVouchers = async () => {
            ...
            try {
                const response = await fetch('http://buitoan.somee.com/api/Vouchers/GetList_SearchVouchers', {
                    method: 'POST',
                    headers,
    ...
    const handleSaveVoucher = async (voucher) => {
        ...
        try {
            const response = await fetch('http://buitoan.somee.com/api/Wallets/Insert_Wallets', {
                method: 'POST',
                headers,
                body: JSON.stringify(data),
            });
    ...
    const handleExchangeClick = async (voucher) => {
        ...
        try {
            const response = await fetch('http://buitoan.somee.com/api/Wallets/RedeemPointsForVoucher', {
                method: 'POST',
                headers,
                body: JSON.stringify(data),
            });
                ...
                {vouchers.map((voucher) => (
                    <div key={voucher.voucherID} className={cx('voucher-card')}>
                        <div className={cx('voucher-card-header')}>
                            <img
                                src={`http://buitoan.somee.com/FilesImages/Vouchers/${voucher.voucherImage}`}
                                alt={voucher.code}
